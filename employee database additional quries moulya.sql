...
FROM emp e
JOIN (SELECT mgr_no, COUNT(*) AS emp_count FROM emp WHERE mgr_no IS NOT NULL
GROUP BY mgr_no ORDER BY emp_count DESC LIMIT 1) m ON e.empno = m.mgr_no;
...
WHERE e.empno IN (
    ...
    GROUP BY mgr_no
    HAVING AVG(sal) < (
        SELECT sal FROM emp WHERE empno = mgr_no
    )
);
...
SELECT e1.*
FROM emp e1
JOIN emp e2 ON e1.mgr_no = e2.empno
